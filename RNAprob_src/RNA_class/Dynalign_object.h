...
class Dynalign_object:  public TwoRNA {
	...
	public:
		...
		Dynalign_object(const char* filename, const int maxtrace, const int bpwin, const int awin, const int percent);
...
#ifdef DYNALIGN_II
	int Dynalign(const int maxtrace, 
                     const int bpwin, const int awin, const int percent, const int imaxseparation=-99, const float slope = 0.1, const float intercept = 0.5, const float gap=0.4, const int max_elongation = 5, 
			const char savefile[]=NULL, const bool optimalonly=false, const int singlefold_subopt_percent=30, const bool local=false, 
			const int numProcessors=1, const int maxpairs=-1);
#else
		int Dynalign(const int maxtrace, 
			const int bpwin, const int awin, const int percent, const int imaxseparation=-99, const float gap=0.4, const bool singleinsert=true, 
			const char savefile[]=NULL, const bool optimalonly=false, const int singlefold_subopt_percent=30, const bool local=false, 
			const int numProcessors=1, const int maxpairs=-1);
#endif
	...
	private:
		...
		int **align;//keep track of the alignment
		int **forcealign;//keep track of any forced alignments
