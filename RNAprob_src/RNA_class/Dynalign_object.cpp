...
Dynalign_object::Dynalign_object(const char* filename, const int maxtrace, const int bpwin, const int awin, const int percent): TwoRNA() {
	int i;
	...
	align = new int *[maxtrace];//maximum number of tracebacks and next line and below at delete
	for (i=0;i<maxtrace;i++)  align[i] = new int [length+1];
#ifdef DYNALIGN_II
...
#ifdef DYNALIGN_II
int Dynalign_object::Dynalign(const int maxtrace, 
                              const int bpwin, const int awin, const int percent, const int imaxseparation, const float slope, const float intercept, const float gap, const int max_elongation, const char savefile[], const bool optimalonly, const int singlefold_subopt_percent, const bool local, 
	const int numProcessors,
      const int maxpairs)
#else
int Dynalign_object::Dynalign(const int maxtrace, 
	const int bpwin, const int awin, const int percent, const int imaxseparation, const float gap, const bool singleinsert, const char savefile[], const bool optimalonly, const int singlefold_subopt_percent, const bool local, 
	const int numProcessors,
      const int maxpairs)
#endif

 {
	...
	int igapincrease = (int)(gap * 10.0);
#ifdef DYNALIGN_II
        int islope = (int)(slope * 10.0);
        int iintercept = (int)(intercept * 10.0);
#else
#endif
	...
	//allocate space for the alignment
	align = new int *[maxtrace];//maximum number of tracebacks and next line and below at delete
	for (i=0;i<maxtrace;i++)  align[i] = new int [GetRNA1()->GetStructure()->GetSequenceLength()+1];
...
void Dynalign_object::AllocateForceAlign() {
	...
	forcealign=new int *[2];

		
	forcealign[0]=new int [GetRNA1()->GetStructure()->GetSequenceLength()+1];
	forcealign[1]=new int [GetRNA2()->GetStructure()->GetSequenceLength()+1];
