...
void Scorer_Interface::run() {
	...
	if( error == 0 ) {
		...
		for( int i = start; i <= end; i++ ) {
			...
			int fp = pairs2 - tp;
			int fn = pairs1 - tp;
			int tn = ((int) (0.5 * length1 * (length1 - 1))) - tp;
			
			double tmp = 1.0*(tp+fp)*(tp+fn)*(tn+fp)*(tn+fn);
			
			double MCC = (1.0*tp*tn - 1.0*fp*fn)/sqrt(tmp) * 100;
			double F1 = 2 * (percent1*percent2)/(percent1+percent2);
			double geometric_mean = sqrt(percent1 * percent2);
			
			// Create the string streams.
			stringstream MCC_stream( stringstream::in | stringstream::out );
			stringstream F1_stream( stringstream::in | stringstream::out );
			stringstream geometric_stream( stringstream::in | stringstream::out );
			stringstream tp_stream( stringstream::in | stringstream::out );
			stringstream fp_stream( stringstream::in | stringstream::out );
			stringstream tn_stream( stringstream::in | stringstream::out );
			stringstream fn_stream( stringstream::in | stringstream::out );
			
			
			// Fill the string streams that hold sensitivity and PPV data.
			MCC_stream << "MCC = " << fixed << setprecision( 2 ) << MCC << "%";
			F1_stream << "F1 = " << fixed << setprecision( 2 ) << F1 << "%";
			geometric_stream << "Geometric_average = " << fixed << setprecision( 2 ) << geometric_mean << "%"; 
			tp_stream << "TP = " << tp;
			fp_stream << "FP = " << fp;
			tn_stream << "TN = " << tn;
			fn_stream << "FN = " << fn;
