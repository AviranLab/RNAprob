...
class pkHelix{
 ...
 public:
	...
	pkHelix(datatable *data, structure *st, int size, int *helixList);
 ...
 private:
	...
	void copy(const pkHelix& f){
		...
		if(f.helix!=0){
			helix=new int[f.length];
			memcpy(helix,f.helix,f.length*sizeof(int));
		}
		else helix=0;
	}
	...
	int *helix;		//pointer to the list of nucleotides
