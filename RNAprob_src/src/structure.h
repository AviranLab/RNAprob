...
int pair[maxforce][2],npair,nforbid,forbid[maxforce][2];//arrays to hold lists of forced pairs or pairs forbidden
int *numseq,*hnumber;
int **basepr;
...
int nnopair,*nopair,nmod,mod[maxforce];
...
int min_gu, min_g_or_u;//NMR-derived constraint variables
int neighbors[maxforce][maxneighborlength],nneighbors;//also NMR-derived index this from zero in both dimensions
//regional NMR constraints:
int nregion,rmin_gu[maxregions],rmin_g_or_u[maxregions];
int rneighbors[maxregions][maxforce][maxneighborlength],rnneighbors[maxregions],start[maxregions],stop[maxregions];
//microarray type constraints:
int nmicroarray,microstart[maxregions],microstop[maxregions],microunpair[maxregions];
...
double *SHAPEss; //int array that contains SHAPE data for single-stranded segments
...
int **SHAPEss_region;  //2-d int array containing energy values for hairpin loop combinations
...
int SHAPEss_give_value(int index);  //Returns the single stranded SHAPE pseudo energy for a given nucleotide
...
//>>>>>>> ../../RNAstructure-object/src/structure.h
	...
	//private:
		...
		//Add by FD
		histSet* trainingParam;
		bool trainingParamRead;
		bool split_by_base;
		bool twoStateVersion;
		void ReadTrainingParam();
		double CalculatePseudoEnergy(double data, std::string modifier, int position, int strucType);
		double *SHAPEdiff;//record the difference between stacked and helix-end pseudo-energy
		//double *SHAPEdiffnew;
		int SHAPEdiff_give_value(int index);
		void printSHAPE();
		void setStateType(bool useTwoState);
		bool smoothVersion;
		void setSmoothVersion(bool useSmoothVersion);
		
		double exponetialDist (double data, double lamb);
		double generalizedExtremeValueDist(double data, double xi, double sigma, double mu);
		double CalculatePseudoEnergy_smooth(double data, int strucType);
