...
//<<<<<<< structure.cpp
	...
	//ADD BY FD
	trainingParam = new histSet();
	trainingParamRead = false;
	split_by_base = false;//if A and C are considered separately for DMS
	twoStateVersion = false;//true for the two-category version, false for the three-category version
	smoothVersion = false; // true is decoded with smoothed version
}
...
//FD
// Set which version to use: two-state or three-state
void structure::setStateType(bool useTwoState)
{
	twoStateVersion = useTwoState;
}

void structure::setSmoothVersion(bool useSmoothVersion)
{
	smoothVersion = useSmoothVersion;
}
...
double structure::exponetialDist (double data, double lamb)
{
	return lamb * exp(-lamb * data);
}


double structure::generalizedExtremeValueDist(double data, double xi, double sigma, double mu)
{
	double oneoverxi = 1/xi;
	double b = 1 + xi * ((data - mu)/sigma);
	double c = pow(b, -1-oneoverxi);
	double d = pow(b, -oneoverxi);
	double result = 1/sigma * c * exp(-d);
	
	return result;
	
}

double structure::CalculatePseudoEnergy_smooth(double data, int strucType)
{
	if (data <= -500)
		return 0;
	else if (data < 0)
	...
	//parameters from Sukosd paper
	double lamb = 1.468;
	double xi_helix = 0.821235;
	double sigma_helix = 0.113916;
	double mu_helix = 0.0901397;
	
	double xi_stacked = 0.762581;
	double sigma_stacked = 0.0492536;
	double mu_stacked = 0.0395857;
	
	double xi_paired = 0.895341;
	...
	if (strucType == STYPE_HELIXEND){
		probability =  generalizedExtremeValueDist(data, xi_helix, sigma_helix, mu_helix);
	}
	else if (strucType == STYPE_STACKED){
		probability =  generalizedExtremeValueDist(data, xi_stacked, sigma_stacked, mu_stacked);
	}
	else if (strucType == STYPE_UNPAIRED){
		probability = exponetialDist(data, lamb);
	}
	else if(strucType == STYPE_PAIRED) {
		probability =  generalizedExtremeValueDist(data, xi_paired, sigma_paired, mu_paired);
	}

	pseudoEnergy =  -(double) rt * log (probability) * conversionfactor;
	if (isnan(pseudoEnergy) == true) {
		return 0;
	}
...
*/
void structure::ReadTrainingParam()
{
	string filedir;
	char *dir=getenv("DATAPATH");
	...
	filedir += "/trainingParam/train_param.txt";
	
	
	//trainingParam = new histSet();
	trainingParam->readHistFile(filedir.c_str());
}

/*
	Add by FD
	calculate pseudoenergy as -rt * log(P(reactivity|strucType))
*/
double structure::CalculatePseudoEnergy(double data, std::string modifier, int position, int strucType)
{
	int dataSource = 0;
	int baseType = 0;
	...
	if (data <= -500)
		...
		data = 0; // set negatives to 0
	
	if (modifier == "SHAPE") 
		dataSource = DSOURCE_SHAPE;
	else if (modifier == "DMS")
		dataSource = DSOURCE_DMS;
	else{
		cout<<"Unexpected data source!\n";
		exit(0);
	}
		
	
	//check if empirical distribution are used by base type 
	if (!split_by_base)
		baseType = BASE_X;
	else {
		if (numseq[position] == 1)
			baseType = BASE_A;
		else if (numseq[position] == 2)
			baseType = BASE_C;
		else if (numseq[position] == 3)
			baseType = BASE_G;
		else if (numseq[position] == 4)
			baseType = BASE_U;
		else{
			//cout<<"Unexpected data source!\n";
			//exit(0);
	...
	//Add code for other data sources, like CMCT
	
	
	if(smoothVersion) {
		return CalculatePseudoEnergy_smooth(data, strucType);
	}
	else {
		ahist = trainingParam->getHistData(dataSource, strucType, baseType);
	
		probability = ahist->getProbability(data);
		if (probability <= 0)
			return 0;
		probability = -(double) rt * log (probability) * conversionfactor;
		return probability;
	}
}
...
void structure::ReadSHAPE(const char *filename, std::string modifier, bool calculate, bool nosum) {
	...
	//Add by FD
	//Only read the histogram parameters if they need to be read.
	if (!trainingParamRead){
		ReadTrainingParam();
		trainingParamRead = true;
	}
...
//		ReadProbabilisticPotentialParams();
		...
		//FD
		//initialize an array to hold the difference between stacked and helix-end pseudoenergy
		SHAPEdiff = new double [2*numofbases+1];
	...
	//FD
	//initializes a temporal array to hold reactivity data for difference between stacked and helix-end
	double *SHAPEdiffnew = new double [2 * numofbases + 1];
	...
	while (!in.eof()) {
		...
		if (position<=numofbases) {
			...
			if (calculate) { 
				...
				//FD
				if (twoStateVersion)//set structure type to be paired when twoStateVersion = true
					SHAPEnew[position] += CalculatePseudoEnergy(data, modifier, position, STYPE_PAIRED);
				else {//use helix-end pseudoenergy 
					double helixE = CalculatePseudoEnergy(data, modifier, position, STYPE_HELIXEND);
					double stackE = CalculatePseudoEnergy(data, modifier, position, STYPE_STACKED);
					SHAPEnew[position] += helixE;
					SHAPEdiffnew[position] += stackE - helixE;
				}
				SHAPEssnew[position] += CalculatePseudoEnergy(data, modifier, position, STYPE_UNPAIRED);
	...
	if (calculate) {
		...
		for (position=1;position<=numofbases;position++) {
			...
			//FD
			SHAPEdiff[position+numofbases] = SHAPEdiff[position];
		}		
	...
	SHAPEss_region = new int *[numofbases + 1];
	for (int i = 1; i <= numofbases; i++) SHAPEss_region[i] = new int [i];

	//fills 2-d array with pseudo energy terms for loops from i-j using ss SHAPE parameters
	for (int j = 2; j <= numofbases; j++) {
		SHAPEss_region[j][j - 1] = (int)(SHAPEss[j] + SHAPEss[j-1]); //sets energy for "zero sized loop".  Acts as starting value to add onto below
		for (int i = j - 2; i >= 1; i--) {
			SHAPEss_region[j][i] = SHAPEss_region[j][i + 1] + (int)(SHAPEss[i]); //adds energy for additional loop element
		}
	}
...
//FD
void structure::printSHAPE()
{
	ofstream of1("shape_helix.txt");
	ofstream of2("shape_stacked.txt");
	ofstream of3("shape_unpaired.txt");
	
	for (int position=1;position<=numofbases;position++) {
			of1<<SHAPE[position]<<endl;
			of2<<SHAPE[position] + SHAPEdiff[position]<<endl;
			of3<<SHAPEss[position]<<endl;
	}
	
	for (int position=1;position<=numofbases;position++) {
			of1<<SHAPE[position + numofbases]<<endl;
			of2<<SHAPE[position+ numofbases] + SHAPEdiff[position+ numofbases]<<endl;
			of3<<SHAPEss[position+ numofbases]<<endl;
	}
...
int structure::SHAPEss_give_value(int index) {
	if (shaped) {
		if (index > numofbases) return (int)SHAPEss[index - numofbases];
		else return (int)SHAPEss[index];
	}
	else return 0;
}


int structure::SHAPEdiff_give_value(int index)
{
	if (shaped) {
		if (index > numofbases) return (int)SHAPEdiff[index - numofbases];
		else return (int)SHAPEdiff[index];
	}
	else return 0;
}
