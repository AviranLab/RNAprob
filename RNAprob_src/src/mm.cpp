...
void twoefn2(datatable *data,structure *ct,int i, int j, int **energy,int count)
{
int k,sum,ip,jp,m,n;
//stackstruct stack;
//bool flag;
...
//dynamically alocate enough space in arrays **subenergy,**subct,and **subcompat
	...
	subenergy=new int *[ct->numofbases+3];
	subcompat=new int *[ct->numofbases+3];
   //subct= new int *[ct->numofbases+3];
   for (i=0;i<=(ct->numofbases+2);i++) {
   	subenergy[i] = new int [ct->numofbases+3];
		subcompat[i] = new int [ct->numofbases+3];
