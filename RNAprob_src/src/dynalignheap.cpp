...
dynalignheap::dynalignheap(int allocate) {
  size=0;
  heapi = new int [allocate];
  heapj = new int [allocate];
  heapa = new int [allocate];
  heapb = new int [allocate];
  max=allocate;
...
void dynalignheap::push(int i, int j, int a, int b, integersize e) {
  dynalignheap *temp;
  int index;
...
void dynalignheap::read(int position, int *i,int *j, int *a, int *b, integersize *e) {
  *i = heapi[position];
  *j = heapj[position];
  *a = heapa[position];
  *b = heapb[position];
