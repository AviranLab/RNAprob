...
void *rankconsumer(void *arg) {
  ...
  int **forcealign = rca->forcealign;
  bool *dbl1 = rca->dbl1;
  bool *dbl2 = rca->dbl2;
  int *lowend = rca->lowend;
  int *highend = rca->highend;
  int gap = rca->gap;
#ifdef DYNALIGN_II
  arrayclass *single1_w = rca->single1_w;
  arrayclass *single2_w = rca->single2_w;
  int max_elongation = rca->max_elongation;
  int islope = rca->islope;
  int iintercept = rca->iintercept;
#else
  bool singleinsert = rca->singleinsert;
#endif
  bool force = rca->force;
  bool local = rca->local;
  int (*edangle5)(int i, int j, int ip, structure *ct, datatable *data) = rca->edangle5;
  int (*edangle3)(int i, int j, int ip, structure *ct, datatable *data) = rca->edangle3;
