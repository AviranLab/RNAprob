...
void findhelix(int **arrayPointer, vector<pkHelix>  &pkHelixList, int x, int y, datatable *data, structure *ct){
	...
	if (helixLength>2){
		...
		//create an array of ints to hold numbers involved in helix
		//use standard array notation 0 to n-1
		int *temphelix;
		//temphelix = new int [2*helixLength-1];//DELETED
		temphelix = new int [2*helixLength];//ADDED
		//fill the temporary array of ints backward since variables hold last elements of helix
...
void convertStructureToHelixList(structure *st, datatable *data, vector<pkHelix> &structureHelices){
	...
	temphelix = new int [st->GetSequenceLength()];
...
void comparepkHelixListToMFE(vector<pkHelix> &pkHelixList, structure *ct, vector<pkHelix> &MFEhelixList){
	...
	//Again, just an empirical rule that allows repairing of int helices
...
void copyStructure(structure * ct, structure* &pseudoStructAggregate, string DMSFile, string SHAPEFile){
	...
	if(!SHAPEFile.empty()||!DMSFile.empty()){
		...
		pseudoStructAggregate->SHAPEss_region = new int *[pseudoStructAggregate->GetSequenceLength() + 1];
		for (int i = 1; i <= pseudoStructAggregate->GetSequenceLength(); i++) {
			pseudoStructAggregate->SHAPEss_region[i] = new int [i];
...
//SHAPEslope and SHAPEintercept are the SHAPE parameters
{
	...
	cout << "\t\t\tDONE\n" << flush;
#endif
	//create the 2d array using the first dimension as an array of pointers to int
	//the second dimension is then an array of ints which grows with each allocation +1 to create an unequal or triangular shape
	
	int **arrayPointer;	 //allocate the pointer to the 2d array for the dot plot
	arrayPointer = new int *[rnaCT->GetSequenceLength()+1];
	for (int t =1; t<=rnaCT->GetSequenceLength();t++)
		arrayPointer[t]=new int [t+1];
