...
ml1>>lineoftext;
 ...
 data->eparam[4] = 0;
 ml1 >> temp;
 data->eparam[5] = (int) (floor (temp*conversionfactor+.5));  //constant multi-loop penalty



 ml1 >> temp;
 data->eparam[6] = (int) (floor (temp*conversionfactor+.5));
 ...
 data->eparam[9] = -500;
 ml1 >> temp;
 data->eparam[10] = (int) (floor (temp*conversionfactor+.5));
 ml1>>lineoftext;
 while(strcmp(lineoftext,"-->")) ml1>>lineoftext;
 ...
 if (ml1.peek()==EOF) {
   ...
   data->singlecbulge = 0;
 	...
 	data->efn2a = (int) (floor (temp*conversionfactor+.5));  //constant multi-loop penalty for efn2

	 ml1 >> temp;
 	data->efn2b= (int) (floor(temp*conversionfactor+.5));

 	ml1 >> temp;
 	data->efn2c= (int) (floor(temp*conversionfactor+.5));

	ml1>>lineoftext;
	while(strcmp(lineoftext,"-->")) ml1>>lineoftext;
	ml1>>temp;
	data->mlasym = (int) (floor (temp*conversionfactor+.5));


	ml1>>lineoftext;
	while(strcmp(lineoftext,"-->")) ml1>>lineoftext;
	ml1>>temp;
	data->strain = (int) (floor (temp*conversionfactor+.5));
   ...
   ml1>> temp;
 	...
 	data->auend = (int) (floor (temp*conversionfactor+.5));
   ...
   ml1>>lineoftext;
 	...
 	ml1 >> temp;
 	data->gubonus = (int) (floor (temp*conversionfactor+.5));
   ...
   ml1>>lineoftext;
 	...
 	ml1 >> temp;
 	data->cslope = (int) (floor (temp*conversionfactor+.5));
   ...
   ml1>>lineoftext;
 	...
 	ml1 >> temp;
 	data->cint = (int) (floor (temp*conversionfactor+.5));
   ...
   ml1>>lineoftext;
 	...
 	ml1 >> temp;
 	data->c3 = (int) (floor (temp*conversionfactor+.5));
   ml1>>lineoftext;
 	while(strcmp(lineoftext,"-->")) ml1>>lineoftext;

   ml1 >> temp;
 	data->init = (int) (floor (temp*conversionfactor+.5));
	...
	ml1>>lineoftext;
 	...
 	ml1>> temp;
 	data->singlecbulge = (int) (floor (temp*conversionfactor+.5));
		...
		for (j=0;j<=5; j++) {
			for (k=0;k<=5; k++) {
                ...
                //cout << lineoftext<<"\n";
					...
					data->dangle[i][j][k][l] = (int) (floor (conversionfactor*(atof(lineoftext))+.5));
				    }
...
for (i=1;i <= 30; i++) {
	...
	lo1 >> lineoftext;
					if (strcmp(lineoftext,".")){
					data->inter[i] = (int) (floor (conversionfactor*(atof(lineoftext))+.5));
					}
	...
	lo1 >> lineoftext;
					if (strcmp(lineoftext,"."))
					data->bulge[i] = (int) (floor(conversionfactor*(atof(lineoftext))+.5));
					else data->bulge[i] = INFINITE_ENERGY;

               //cout <<"bulge = "<<data->bulge[i]<<"\n";
	lo1 >> lineoftext;
					if (strcmp(lineoftext,".")){
					data->hairpin[i] = (int) (floor(conversionfactor*(atof(lineoftext))+.5));
					}
		...
		for (j=0;j<=5;j++) {
			for (l=0;l<=5;l++) {
				if ((i==0)||(j==0)||(k==0)||(l==0)) {
				    ...
				    th1 >> lineoftext;
				    if (strcmp(lineoftext,".")){
					    data->tstkh[i][j][k][l] =(int) (floor(conversionfactor*(atof(lineoftext))+.5));
				    }
		...
		for (j=0;j<=5;j++) {
			for (l=0;l<=5;l++) {
                ...
                //cout <<lineoftext<<"\n";
				    ...
				    if (strcmp(lineoftext,".")){
					data->tstki[i][j][k][l] =(int) (floor (conversionfactor*(atof(lineoftext))+.5));
				    }
		...
		for (j=0;j<=5;j++) {
			for (l=0;l<=5;l++) {
                ...
                //cout <<lineoftext<<"\n";
				    ...
				    if (strcmp(lineoftext,".")){
					data->tstki23[i][j][k][l] =(int) (floor (conversionfactor*(atof(lineoftext))+.5));
				    }
		...
		for (j=0;j<=5;j++) {
			for (l=0;l<=5;l++) {
                ...
                //cout <<lineoftext<<"\n";
				    ...
				    if (strcmp(lineoftext,".")){
					data->tstki1n[i][j][k][l] =(int) (floor (conversionfactor*(atof(lineoftext))+.5));
				    }
...
//cout << data->tloop[data->numoftloops][0] << "\n";
	tl1 >> temp;
	data->tloop[data->numoftloops][1] = (int) (floor (conversionfactor*temp+0.5));
...
for (i=1;i<=36;i++) {//read each of 36 tables
	...
	for (j=1;j<=4;j++) {
		...
		for (l=1;l<=4;l++) {
		    for (m=1;m<=4;m++) {
			in1 >> temp;
			data->iloop22[a][b][c][d][j][l][k][m] = (int) (floor(conversionfactor*temp+0.5));
	...
	for (e=1;e<=4;e++) {
		...
		for (c=1;c<=4;c++) {
			for (j=1;j<=6;j++) {
				...
				for (d=1;d<=4;d++) {
					in2 >> temp;
					data->iloop21[a][b][c][d][e][f][g]=int (floor(conversionfactor*temp+0.5));
					//cout << a<<" "<<b<<" "<<c<<" "<<d<<" "<<e<<" "<<f<<" "<<g<<"\n";
               //cout << temp<<"\n";
...
for (count=1;count<=maxtloop&&!tri.eof();count++){
	...
	data->triloop[data->numoftriloops][0] = data->triloop[data->numoftriloops][0]+
		625*tonumi(base);
	tri >> temp;
	data->triloop[data->numoftriloops][1] = (int) (floor (conversionfactor*temp+0.5));
		...
		for (j=0;j<=5;j++) {
			for (l=0;l<=5;l++) {
               ...
               //cout << lineoftext <<"end\n";
					...
					if (strcmp(lineoftext,".")){
						data->coax[j][i][k][l] =int (floor(conversionfactor*(atof(lineoftext))+.5));
					}
		...
		for (j=0;j<=5;j++) {
			for (l=0;l<=5;l++) {
				if ((i==0)||(j==0)||(k==0)||(l==0)||(i==5)||(j==5)||(k==5)||(l==5)) {
				    ...
				    co2 >> lineoftext;
				    if (strcmp(lineoftext,".")){
					    data->tstackcoax[i][j][k][l] =int (floor(conversionfactor*(atof(lineoftext))+.5));
				    }
		...
		for (j=0;j<=5;j++) {
			for (l=0;l<=5;l++) {
				if ((i==0)||(j==0)||(k==0)||(l==0)||(i==5)||(j==5)||(k==5)||(l==5)) {
				    ...
				    co3 >> lineoftext;
				    if (strcmp(lineoftext,".")){
					    data->coaxstack[i][j][k][l] =int (floor(conversionfactor*(atof(lineoftext))+.5));
				    }
			...
			for (l=0;l<=5;l++) {
            	...
            	if ((k==5)&&(l==5)) {//flush end
               	...
               	data->tstack[i][j][k][l] = data->dangle[i][j][k][1];
				    ...
				    st2 >> lineoftext;
				    if (strcmp(lineoftext,".")){
					data->tstack[i][j][k][l] =int (floor (conversionfactor*(atof(lineoftext))+.5));
				    }
			...
			for (l=0;l<=5;l++) {
				if ((i==0)||(j==0)||(k==0)||(l==0)) {
             	...
             	data->tstkm[i][j][k][l] = INFINITE_ENERGY;
				    ...
				    tsm >> lineoftext;
				    if (strcmp(lineoftext,".")){
					    data->tstkm[i][j][k][l] =int (floor(conversionfactor*(atof(lineoftext))+.5));
				    }
   ...
   for (b=1;b<=4;b++) {
   	for (j=1;j<=6;j++) {
      	if (j==1) {
   		...
   		for (e=1;e<=4;e++) {
         	i11 >> temp;
            data->iloop11[a][b][c][d][e][f]=int (floor(conversionfactor*temp+0.5));
...
//ADD BY FD
//get pseudo-energy for a pair
integersize SHAPEendPair(int i, int j, structure *ct) {
	if (ct->shaped) {
		return SHAPEend(i,ct) + SHAPEend(j,ct);
		//return (integersize) (ct->SHAPE[i]+ct->SHAPE[j]);
	}
	return 0;
	//return SHAPEend(i,ct)+SHAPEend(j,ct);
}
...
integersize erg1(int i,int j,int ip,int jp,structure *ct, datatable *data)
{
		...
		integersize energy;
		 ...
		 if ((i==(ct->GetSequenceLength()))||(j==((ct->GetSequenceLength())+1))) {
			...
			//the following 4 lines are commentted out by FD
...
integersize ergmulti(int st, int ip, structure *ct, datatable *data, bool simplemb) {
	int *element,**energy;
	int i,count,b,c,size,j,minimum,au,current,first,recent,biggest;
	bool intermolecular;
	float average;
	...
	element = new int [count+3];
	energy = new int *[count+3];
	for (i=0;i<count+3;i++) {
		energy[i]=new int [count+3];
		for (j=0;j<count+3;j++)
			energy[i][j]=0;
	...
	minimum = minimum+(int) (((float) data->mlasym)*average);
...
integersize ergexterior(int st, structure *ct, datatable *data) {
	int *element,**energy;
	int i,count,size,j,minimum,au,helices;
	bool intermolecular;
